...
보통 이상치 탐색은 boxplot 시각화를 통해 확인하고 검출해 낼 수 있지만, feature가 많고 데이터 수가 많은 데이터 셋에서는 하나하나 살펴보기가 어렵다. 이를 위해 여기서는 percentile을 활용해 갹갹의 feature 내에 이상치를 갖는 관측치들을 추출하여 21개의 features 가운데 5개 features 이상에서 이상치가 존재하는 관측치는 제거했다. 

코드를 활용함에 있어 주의할 점은 outlier_indices의 list에서 .append가 아닌 extend를 써야하는 점이다. append()와 extend()의 차이점을 보면 다음 아래와 같다.   
...
5개의 features에서 이상치가 존재하는 356개의 관측치를 제거하면 총 199635 관측치가 남게 되는 것으로 확인할 수 있다. 
<br/>
<br/>
